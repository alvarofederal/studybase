...
export default function MateriasCliente({ materias, statusInicial }: Props) {
  ...
  return (
    <div>
      {/* Banner de erro (ex: limite de matérias) */}
      {erro && (
        <div className="flex items-start gap-3 mb-5 bg-red-500/10 border border-red-500/30 rounded-xl px-4 py-3">
          <span className="text-red-400 text-base shrink-0 mt-0.5">⚠️</span>
          <div className="flex-1 min-w-0">
            <p className="text-sm text-red-400 font-medium leading-snug">{erro}</p>
            <p className="text-xs text-red-400/60 mt-1">
              Acesse{" "}
              <Link href="/estudo/perfil" className="underline underline-offset-2 hover:text-red-300 transition-colors">
                Meu Perfil
              </Link>{" "}
              para remover uma matéria e liberar uma vaga.
            </p>
          </div>
          <button
            onClick={() => setErro(null)}
            className="text-red-400/50 hover:text-red-400 transition-colors shrink-0 leading-none mt-0.5"
            aria-label="Fechar"
          >
            ✕
          </button>
